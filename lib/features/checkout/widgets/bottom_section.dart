 ...
 */
  ...
  Widget pricingView({required BuildContext context, required bool takeAway}) {
    ...
    /*(parsedPdate != null && (parsedPdate.isAfter(parsedDate) || parsedPdate.isAtSameMomentAs(parsedDate))  && (serviceenable == 1) && (serviceorders! == userorders! || userorders! > serviceorders!) && (userpremium! == premium! )  ) ? Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(Get.find<SplashController>().configModel!.serviceChargeName!, style: robotoRegular),
              Text(
                '(+) ${PriceConverter.convertPrice(Get.find<SplashController>().configModel!.serviceCharge)}',
                style: robotoRegular, textDirection: TextDirection.ltr,
              ),
            ]) : const SizedBox(),
            SizedBox(height: (parsedPdate != null && (parsedPdate.isAfter(parsedDate) || parsedPdate.isAtSameMomentAs(parsedDate))  && (serviceenable == 1) && (serviceorders! == userorders! || userorders! > serviceorders!) && (userpremium! == premium! )  ) ? Dimensions.paddingSizeSmall : 0),


     */

            //
            (checkoutController.store!.storeChargesStatus!) ? Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('Store Charges'.tr, style: robotoRegular),
                Text('(+) ${PriceConverter.convertPrice(Get.find<SplashController>().configModel!.storeCharge)}', style: robotoRegular, textDirection: TextDirection.ltr),
              ],
            ) : const SizedBox.shrink(),
            SizedBox(height: checkoutController.store!.storeChargesStatus! ? Dimensions.paddingSizeSmall : 0),
