...
import 'package:sixam_mart/features/subscription/domain/models/subscription_model.dart';
    ...
    return Scaffold(
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn() ? GetBuilder<CheckoutController>(builder: (checkoutController) {
        ...
        /*  DateTime dateToday = DateTime.now();
          ...
          int? serviceorders = Get.find<SplashController>().configModel!.serviceOrderLimit!;
          int? userorders = Get.find<ProfileController>().userInfoModel?.premiumuserorders;
          double? userpremium = Get.find<ProfileController>().userInfoModel?.premium;
          double? premium = Get.find<SplashController>().configModel?.premium1Rate;
          int? serviceenable = Get.find<SplashController>().configModel?.serviceChargeStatus;


          double serviceCharge =  (parsedPdate != null && (parsedPdate.isAfter(parsedDate) || parsedPdate.isAtSameMomentAs(parsedDate))  && (serviceenable == 1) && (serviceorders! == userorders! || userorders! > serviceorders!) && (userpremium! == premium! )  )
              ? Get.find<SplashController>().configModel!.serviceCharge! : 0;
...
// Safely parse premium expiry date
          DateTime? parsedPdate = (pdateNullable != null && pdateNullable.isNotEmpty)
              ? DateTime.tryParse(pdateNullable)
              : null;
          ...
          int? userOrders = Get.find<ProfileController>().userInfoModel?.premiumuserorders;
          double? userPremium = Get.find<ProfileController>().userInfoModel?.premium;
          double? premiumRate = Get.find<SplashController>().configModel?.premium1Rate;
          int? serviceEnable = Get.find<SplashController>().configModel?.serviceChargeStatus;
          double? serviceChargeConfig = Get.find<SplashController>().configModel?.serviceCharge;

// Ensure all required values are non-null before applying conditions
          bool isPremiumValid = parsedPdate != null &&
              (parsedPdate.isAfter(parsedDate) || parsedPdate.isAtSameMomentAs(parsedDate));
          bool isServiceEnabled = serviceEnable == 1;
          bool isOrderLimitExceeded = serviceOrders != null && userOrders != null &&
              (userOrders >= serviceOrders);
          bool isPremiumMatch = userPremium != null && premiumRate != null &&
              userPremium == premiumRate;

// Calculate service charge
          double serviceCharge = (isPremiumValid && isServiceEnabled && isOrderLimitExceeded && isPremiumMatch)
              ? (serviceChargeConfig ?? 0)
              : 0;
...
*/
          ...
          double storeCharge =  checkoutController.store!.storeChargesStatus!
              ? Get.find<SplashController>().configModel!.storeCharge! : 0;
          ...
          double originalCharge = _calculateOriginalDeliveryCharge(
            ...
            distance: checkoutController.distance, extraCharge: checkoutController.extraCharge, subTotal: subTotal,
          );
          double deliveryCharge = _calculateDeliveryCharge(
            ...
            extraCharge: checkoutController.extraCharge, orderType: checkoutController.orderType!, orderAmount: orderAmount, subTotal: subTotal,
          );
          ...
          double total = _calculateTotal(
            ...
            tips: checkoutController.tips, additionalCharge: additionalCharge, storeCharge: storeCharge, extraPackagingCharge: extraPackagingCharge,
          );
  ...
  double _calculateOriginalDeliveryCharge({required Store? store, required AddressModel address, required double? distance, required double? extraCharge, required dynamic subTotal }) {
...
/*
    ...
    DateTime? parsedPdate = (pdateNullable != null && pdateNullable.isNotEmpty)
        ...
        : null;
    DateTime parsedDate = DateTime.parse(date);
    int? premiumorders = Get.find<ProfileController>().userInfoModel?.premiumorders;
    int? userorders = Get.find<ProfileController>().userInfoModel?.premiumuserorders;
    int? premiumklm = Get.find<ProfileController>().userInfoModel?.premiumklm ;
    int? Premiumminvalue = Get.find<ProfileController>().userInfoModel?.premiumminvalue ;
     

      if(parsedPdate != null && (parsedPdate.isAfter(parsedDate) || parsedPdate.isAtSameMomentAs(parsedDate)) && (premiumorders! == userorders! || premiumorders! > userorders!)  &&  (premiumklm! == distance! ||  premiumklm! > distance! ) && (Premiumminvalue! <= subTotal!)) {
      deliveryCharge = 0;
    }
    else {
...
// Ensure all required values are non-null before applying conditions
    bool upremium = (userpremium == premium) ;

    bool isPremiumValid = parsedPdate != null &&
        (parsedPdate.isAfter(parsedDate) || parsedPdate.isAtSameMomentAs(parsedDate));

    bool isOrderValid = (premiumOrders != null && userOrders != null) &&
        (userOrders <= premiumOrders);

    bool isDistanceValid = (premiumKlm != null && distance != null) &&
        (distance <= premiumKlm);

    bool isMinValueValid = (premiumMinValue != null && subTotal != null) &&
        (subTotal >= premiumMinValue);

// Set delivery charge conditionally

    if (isPremiumValid && isOrderValid && isDistanceValid && isMinValueValid && upremium){
      deliveryCharge = deliveryCharge * 0.5; // Reduces deliveryCharge by 50%
    }else if (isPremiumValid && isOrderValid && isDistanceValid && isMinValueValid) {
      deliveryCharge = 0;
    }
  ...
  double _calculateDeliveryCharge({required Store? store, required AddressModel address, required double? distance, required double? extraCharge, required double orderAmount, required String orderType, required dynamic subTotal}) {
    double deliveryCharge = _calculateOriginalDeliveryCharge(store: store, address: address, distance: distance, extraCharge: extraCharge, subTotal: subTotal);
  ...
  double _calculateTotal({
    ...
    required String orderType, required double tips, required double additionalCharge,required double storeCharge, required double extraPackagingCharge,
  }) {
    ...
    return PriceConverter.toFixed(
        subTotal + deliveryCharge - discount- couponDiscount + (taxIncluded ? 0 : tax)
            ...
            + additionalCharge + storeCharge + extraPackagingCharge
    );
  }
