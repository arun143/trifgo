...
class PremiumRepository implements PremiumRepositoryInterface{
  final ApiClient apiClient;
  final SharedPreferences sharedPreferences;
  PremiumRepository( {required this.apiClient, required this.sharedPreferences});
  ...
  Future<Response> addFundToWallet(double amount, String paymentMethod) async {
    ...
    return await apiClient.postData(AppConstants.addPremiumUri,
      {
        "amount": amount,
        "payment_method": paymentMethod,
        "payment_platform": GetPlatform.isWeb ? 'web' : '',
        "callback": '$protocol//$hostname${RouteHelper.getPremiumRoute()}',
      }
    );
  }
