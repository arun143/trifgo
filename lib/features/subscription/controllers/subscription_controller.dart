...
import 'package:sixam_mart/features/subscription/widgets/trial_end_bottomsheet.dart';
...
import 'package:sixam_mart/helper/date_converter_helper.dart';
...
class SubscriptionController extends GetxController implements GetxService {
  ...
  SubscriptionModel? get configModel => _subscriptionModel;
  ...
  Future<ResponseModel?> renewBusinessPlan({required String storeId, required bool isCommission}) async {
    ...
    if (response.statusCode == 200) {
      if(response.body['redirect_link'] != null) {
        ...
        Get.back();
       // Get.toNamed(RouteHelper.getPaymentRoute(digitalPaymentName, redirectUrl,  null, false, null, guestId: ''));
      } else {
        _renewStatus = 'packages';
        await Get.find<ProfileController>().getUserInfo();
        getProfile(Get.find<ProfileController>().userInfoModel);
        Get.back();
        Get.back();
  ...
  Future<void> cancelSubscription(int storeId, int subscriptionId) async {
    ...
    if(response.statusCode == 200) {
      await Get.find<ProfileController>().getUserInfo();
      await getProfile(Get.find<ProfileController>().userInfoModel);
      Get.back();
  ...
  Future<void> getProfile(UserInfoModel? proModel) async {
  ...
  Future<void> getSubscriptionTransactionList({required String offset, required String? from, required String? to, required String? searchText}) async {
    ...
    if (!_offsetList.contains(offset)) {
      ...
      SubscriptionTransactionModel? subscriptionTransactionModel = await subscriptionServiceInterface.getSubscriptionTransactionList(
        ...
        restaurantId: Get.find<ProfileController>().userInfoModel!.userInfo!.id, searchText: searchText,
      );
  ...
  /*Future<bool> trialEndBottomSheet() async {
    if(Get.find<ProfileController>().userInfoModel != null && Get.find<ProfileController>().userInfoModel!.userInfo!.storeBusinessModel != 'commission' && Get.find<ProfileController>().userInfoModel!.subscription?.status == 0) {
      Future.delayed(const Duration(seconds: 1), () {
        showModalBottomSheet(
          context: Get.context!, isScrollControlled: true, backgroundColor: Colors.transparent,
          builder: (con) => TrialEndBottomSheet(isTrial: Get.find<ProfileController>().userInfoModel!.subscription?.isTrial == 1),
        );
      });
      return false;
    } else {
