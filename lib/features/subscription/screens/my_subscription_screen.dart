...
import 'package:sixam_mart/common/widgets/custom_app_bar_widget.dart';
...
import 'package:sixam_mart/features/subscription/widgets/change_subscription_plan_bottom_sheet.dart';
//import 'package:sixam_mart/features/subscription/widgets/subscription_details_widget.dart1';
import 'package:sixam_mart/features/subscription/widgets/transaction_widget.dart';
...
class _MySubscriptionScreenState extends State<MySubscriptionScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if(Get.find<AuthController>().isLoggedIn()) {
      Get.find<SubscriptionController>().getProfile(Get.find<ProfileController>().userInfoModel);
    } else {
     Get.find<SubscriptionController>().getProfile(Get.find<AuthController>().login as UserInfoModel?);
    }
    ...
    return GetBuilder<SubscriptionController>(builder: (subscriptionController) {
      ...
      return PopScope(
        ...
        child: Scaffold(
          ...
          body: subscriptionController.profileModel != null ? (businessIsCommission) ? Column(children: [
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  child: Column(children: [
                    ...
                    Text(
                      '${Get.find<ProfileController>().userInfoModel != null && Get.find<ProfileController>().userInfoModel!.userInfo?.phone != null && Get.find<ProfileController>().userInfoModel!.userInfo!.id! > 0
          ...
          ]) : (businessIsNone || (businessIsUnsubscribed && (subscriptionController.profileModel?.userBusinessModel == null))) ? Column(children: [
            ...
            Container(
              padding: const EdgeInsets.all(Dimensions.paddingSizeDefault),
              child: CustomButtonWidget(
                buttonText: (businessIsNone || (businessIsUnsubscribed && (subscriptionController.profileModel?.userBusinessModel == null))) ? 'chose_business_plan'.tr : 'change_business_plan'.tr,
                radius: Dimensions.radiusDefault,
                height: 55,
                onPressed: () {
                  showCustomBottomSheet(
