...
class BannerRepository implements BannerRepositoryInterface {
  ...
  @override
  Future getList({int? offset, bool isBanner = false, bool isTaxiBanner = false, bool isFeaturedBanner = false, bool isParcelOtherBanner = false, bool isPromotionalBanner = false, bool isAddBanner = false, DataSourceEnum? source}) async {
    if (isBanner) {
      ...
      return await _getAddBannerList();
    }
  }
  ...
  Future<PromotionalBanner?> _getAddBannerList() async {
    PromotionalBanner? promotionalBanner;
    Response response = await apiClient.getData(AppConstants.adsContentUri);
    if (response.statusCode == 200 && response.body is Map) {
      promotionalBanner = PromotionalBanner.fromJson(response.body);
    }
    return promotionalBanner;
