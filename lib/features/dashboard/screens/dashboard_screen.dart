...
import 'package:sixam_mart/features/dashboard/widgets/premium_bottom_sheet_widget.dart';
import 'package:sixam_mart/features/premium/controllers/premium_controller.dart';
...
class DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> suggestPremiumBottomSheet() async {
    ...
    if(widget.fromSplash && Get.find<LocationController>().showLocationSuggestion && active) {
      Future.delayed(const Duration(seconds: 1), () {
        showModalBottomSheet(
          context: Get.context!, isScrollControlled: true, backgroundColor: Colors.transparent,
          builder: (con) => const PremiumBottomSheetWidget(),
        ).then((value) {
          Get.find<PremiumController>().hideSuggestedPremium();
          setState(() {});
        });
      });
    }
